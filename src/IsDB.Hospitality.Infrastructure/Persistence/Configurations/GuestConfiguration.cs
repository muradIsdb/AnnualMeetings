...
public class GuestConfiguration : IEntityTypeConfiguration<Guest>
{
    public void Configure(EntityTypeBuilder<Guest> builder)
    {
        ...
        builder.HasMany(g => g.TravelBookings).WithOne(tb => tb.Guest).HasForeignKey(tb => tb.GuestId).OnDelete(DeleteBehavior.Cascade);
...
public class TravelBookingConfiguration : IEntityTypeConfiguration<TravelBooking>
{
    public void Configure(EntityTypeBuilder<TravelBooking> builder)
    {
        ...
        builder.Property(tb => tb.SeatClass).HasMaxLength(50);
        builder.Property(tb => tb.BookingNotes).HasMaxLength(1000);
...
public class DepartureRequestConfiguration : IEntityTypeConfiguration<DepartureRequest>
{
    public void Configure(EntityTypeBuilder<DepartureRequest> builder)
    {
        ...
        builder.Property(dr => dr.FullName).IsRequired().HasMaxLength(200);
        builder.Property(dr => dr.Email).IsRequired().HasMaxLength(200);
        builder.HasIndex(dr => dr.Email).IsUnique();
        builder.Property(dr => dr.RoomNumber).IsRequired().HasMaxLength(50);
        builder.Property(dr => dr.ManageToken).IsRequired();
        builder.HasIndex(dr => dr.ManageToken).IsUnique();

        builder.HasOne(dr => dr.HotelOption).WithMany().HasForeignKey(dr => dr.HotelOptionId).OnDelete(DeleteBehavior.Restrict);
        builder.HasOne(dr => dr.PickupDayOption).WithMany().HasForeignKey(dr => dr.PickupDayOptionId).OnDelete(DeleteBehavior.Restrict);
        builder.HasOne(dr => dr.PickupHourOption).WithMany().HasForeignKey(dr => dr.PickupHourOptionId).OnDelete(DeleteBehavior.Restrict);
    }
}
