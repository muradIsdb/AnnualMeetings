...
public class EventsAirClient : IEventsAirClient
{
    ...
    public async Task<List<EventsAirTravelDto>> GetTravelBookingsAsync(CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("Fetching travel bookings from EventsAir for event: {EventCode}", _options.EventCode);

        var query = $$"""
        {
          "query": "query GetTravelBookings($eventId: ID!) { event(id: $eventId) { travelBookings(limit: 2000) { id contact { id } travelType { name } flightNumber carrier { name } arrivalDate departureDate eta etd departurePort { name } arrivalPort { name } class bookingNotes comment } } }",
          "variables": { "eventId": "{{_options.EventCode}}" }
        }
        """;
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogError("EventsAir travel bookings API returned {StatusCode}", response.StatusCode);
                return new List<EventsAirTravelDto>();
            }

            var json = await response.Content.ReadAsStringAsync(cancellationToken);
            return ParseTravelBookingsResponse(json);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to fetch travel bookings from EventsAir");
            return new List<EventsAirTravelDto>();
        }
    }

    private List<EventsAirTravelDto> ParseTravelBookingsResponse(string json)
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning("EventsAir GraphQL errors: {Errors}", errors.ToString());
                return result;
            }
                ...
                eventObj.TryGetProperty("travelBookings", out var bookings))
            {
                foreach (var booking in bookings.EnumerateArray())
                {
                    var dto = new EventsAirTravelDto
                    {
                        ...
                        ContactId = booking.TryGetProperty("contact", out var contact) ? contact.GetProperty("id").GetString() ?? string.Empty : string.Empty,
                        TravelTypeName = booking.TryGetProperty("travelType", out var tt) ? tt.GetProperty("name").GetString() : null,
                        FlightNumber = booking.TryGetProperty("flightNumber", out var fn) ? fn.GetString() : null,
        ...
        {
            _logger.LogError(ex, "Error parsing EventsAir travel bookings response");
        }
        return result;
    }
