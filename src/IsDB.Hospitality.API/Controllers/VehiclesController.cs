...
public class VehiclesController : ApiControllerBase
{
    ...
    {
        // ── Sync-log view: return SyncAlerts data instead of vehicle list ──
        if (view == "sync-log")
        {
            var role2 = User.FindFirst("role")?.Value ?? "";
            if (role2 != "Admin" && role2 != "Transport") return Forbid();
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                ...
                q = q.Where(a =>
                    (a.GuestName != null && a.GuestName.ToLower().Contains(s2)) ||
                    (a.VehiclePlate != null && a.VehiclePlate.ToLower().Contains(s2)) ||
                    (a.EventsAirContactId != null && a.EventsAirContactId.ToLower().Contains(s2)));
            }
            var total2 = await q.CountAsync(ct);
            var items2 = await q
                ...
                .ToListAsync(ct);
            return Ok(new { items = items2, totalCount = total2, totalPages = (int)Math.Ceiling(total2 / (double)pageSize), page, pageSize });
        }
        if (view == "sync-log-summary")
        {
            var role3 = User.FindFirst("role")?.Value ?? "";
            if (role3 != "Admin" && role3 != "Transport") return Forbid();
        ...
        if (view == "sync-log-resolve" && tab != null && Guid.TryParse(tab, out var resolveId))
        {
            var role4 = User.FindFirst("role")?.Value ?? "";
            if (role4 != "Admin" && role4 != "Transport") return Forbid();
            var alert2 = await _db.SyncAlerts.FindAsync(new object[] { resolveId }, ct);
            if (alert2 == null) return NotFound();
            var userName2 = User.FindFirst("name")?.Value ?? User.FindFirst("sub")?.Value ?? "Unknown";
            alert2.IsResolved = true; alert2.ResolvedAt = DateTime.UtcNow; alert2.ResolvedByUserName = userName2;
            if (!string.IsNullOrWhiteSpace(search)) alert2.Notes = search;
            await _db.SaveChangesAsync(ct);
            return Ok(new { success = true });
        }
        if (view == "sync-log-resolve-all")
        {
            var role5 = User.FindFirst("role")?.Value ?? "";
            if (role5 != "Admin" && role5 != "Transport") return Forbid();
            var userName3 = User.FindFirst("name")?.Value ?? User.FindFirst("sub")?.Value ?? "Unknown";
            var qAll = _db.SyncAlerts.Where(a => !a.IsResolved);
            if (!string.IsNullOrWhiteSpace(tab) && Enum.TryParse<SyncAlertType>(tab, true, out var atAll))
                qAll = qAll.Where(a => a.AlertType == atAll);
            var alertsAll = await qAll.ToListAsync(ct);
            var nowAll = DateTime.UtcNow;
            foreach (var a in alertsAll) { a.IsResolved = true; a.ResolvedAt = nowAll; a.ResolvedByUserName = userName3; }
            await _db.SaveChangesAsync(ct);
            return Ok(new { resolved = alertsAll.Count });
        }
        if (view == "sync-log-delete" && tab != null && Guid.TryParse(tab, out var deleteId))
        {
            var role6 = User.FindFirst("role")?.Value ?? "";
            if (role6 != "Admin") return Forbid();
            var alertDel = await _db.SyncAlerts.FindAsync(new object[] { deleteId }, ct);
            if (alertDel == null) return NotFound();
            _db.SyncAlerts.Remove(alertDel);
            await _db.SaveChangesAsync(ct);
            return NoContent();
        }
        // ── Normal vehicle list ──────────────────────────────────────────────
